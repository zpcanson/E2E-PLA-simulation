...
P_theoretical = zeros(length(Z_Value),length(SNR_dB));
Pmd_theory_h_d = zeros(length(Z_Value),length(SNR_dB));
Pfa_theoretical = zeros(length(Z_Value),length(SNR_dB));

for ka_indx=1:length(Z_Value)
    Z=Z_Value(ka_indx);
    kappa_h = 0;
    kappa_tau = 0;
...
plot(SNR_dB,Pm_d(3,:),'-+k','LineWidth',2,'MarkerSize',10);
 plot(SNR_dB,Pfa_theoretical(1,:),'-sk','LineWidth',2,'MarkerSize',10);
 plot(SNR_dB,Pfa_theoretical(2,:),'--Xk','LineWidth',2,'MarkerSize',10);
...
leg = legend('Z=1 P_{md}','Z=3 P_{md}','Z=5 P_{md}','Z=1 P_{fa}','Z=3 P_{fa}','Z=5 P_{fa}');
text(6,0.9,'\kappa_h=0dB','fontsize',30);
text(6,0.75,'\delta_h=1, \delta_{\tau}=2','fontsize',30);
set(gca,'box','off');
set(leg,'box','off');

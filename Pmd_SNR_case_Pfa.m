...
%% scenarios  
    ...
    SNR=10.^(SNR_dB/10);
P_theoretical = zeros(length(fade_scn_Value),length(SNR_dB));
Pmd_theory_h_d = zeros(length(fade_scn_Value),length(SNR_dB));
Pfa_theoretical = zeros(length(fade_scn_Value),length(SNR_dB));


 for ka_indx=1:length(fade_scn_Value) 
     
    fade_scn=fade_scn_Value(ka_indx);
...
set(0,'defaultfigurecolor','w');
  ...
  plot(SNR_dB,Pm_d(1,:),'-*k','LineWidth',1,'MarkerSize',10);
  plot(SNR_dB,Pm_d(2,:),'ks--','LineWidth',1,'MarkerSize',10);
  plot(SNR_dB,Pm_d(3,:),'-.kX','LineWidth',1,'MarkerSize',10);
...
leg = legend('Case I P_{md}','Case II P_{md}','Case III P_{md}','Case I P_{fa}','Case II P_{fa}','Case III P_{fa}');
text(8,0.9,'Z=1, \kappa_h=0dB','fontsize',30);

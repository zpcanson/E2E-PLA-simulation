...
Pd_theory_h_1D = zeros(length(kappa_h_Value),length(SNR_dB));
Pmd_theory_h_1D = zeros(length(kappa_h_Value),length(SNR_dB));
Pfa_1D = zeros(length(kappa_h_Value),length(SNR_dB));
...
%
  ...
  if Z>=L-1  
   ...
   %   Pmd_theory_h_d(ka_indx,i) = P_theoretical(ka_indx,i);
      ...
      for z=Z+1:L
      Pd_theory_h_1D ( ka_indx,i) = Pd_theory_h_1D ( ka_indx,i)+nchoosek(L,z)*Ph_H1(i)^(z)*(1-Ph_H1(i))^(L-z);
      end
  
      Pmd_theory_h_1D ( ka_indx,i) =1-Pd_theory_h_1D ( ka_indx,i);
 ...
 % Pfa_theoretical(i)=Pfa_theoretical1;
  ...
  num_pd_1D(ka_indx,i)=sum(numerr_1D(i,:));
  Pmd_simu_1D(ka_indx,i)=1-num_pd_1D(ka_indx,i)/(N-1);
end
...
%
  ...
  if Z>L-1  
    ...
    for z=Z+1:L
      Pfa_1D( ka_indx,i)=Pfa_1D( ka_indx,i)+nchoosek(L,z)*Ph_H0(i)^(z)*(1-Ph_H0(i))^(L-z);
    end
...
%         power_d_I_a(ii) = mean(d_I_ar(ii,:).*conj(d_I_ar(ii,:)));
 ...
 % Pfa_theoretical(i)=Pfa_theoretical1;   
  ...
  num_errFA_1D(ka_indx,i)=sum(numerrFA_1D(i,:));
  Pfa_simu_1D(ka_indx,i)=num_errFA_1D(ka_indx,i)/(N-1);
end
...
set(0,'defaultfigurecolor','w');
  ...
  plot(SNR_dB,Pmd_theory_h_1D(1,:),'--k','LineWidth',2,'MarkerSize',10);

 
 plot(SNR_dB,Pfa_theoretical(1,:),'+-k','LineWidth',1.5,'MarkerSize',9);
 plot(SNR_dB,Pfa_1D(1,:),':.k','LineWidth',2,'MarkerSize',10);
 
 
  plot(SNR_dB,Pmd_simu_h_d(1,:),'ok','LineWidth',2,'MarkerSize',10);
  plot(SNR_dB,Pmd_simu_1D(1,:),'ok','LineWidth',2,'MarkerSize',10);

 plot(SNR_dB,Pfa_simu(1,:),'ok','LineWidth',2,'MarkerSize',10);
 plot(SNR_dB,Pfa_simu_1D(1,:),'ok','LineWidth',2,'MarkerSize',10);
...
leg=legend('CA-DI Theory P_{md}','CA Theory P_{md}','CA-DI Theory P_{fa}','CA Theory P_{fa}','Simulation');

text(4,0.9,'Z=3, \kappa_h=0dB','fontsize',30);
text(4,0.75,'\delta_h=1, \delta_{\tau}=2','fontsize',30);
